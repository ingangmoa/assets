...
function CalendarInline(id) {
    ...
    if (this.calendar) {
        flatpickr(this.calendar.querySelector('.js-calendar-inline'), this.calendarOptions);
    }
}
...
function ProductCart() {
    ...
    this.products.forEach((el, index, arr) => {
        const product = el;
...
ProductCart.counter = (event, priceElement, priceSymbol, priceNumber) => {
...
ProductCart.prototype.removeItem = function (event, target) {
    this.productsLenght--;
...
function formEditor() {
    ...
    formEditorIcons.background = `<svg viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
...
InputSelects.destroySearch = event => {
    const element = event.currentTarget;
